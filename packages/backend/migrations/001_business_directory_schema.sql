-- Migration: Business Directory Schema
...
ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'customer' CHECK (role IN ('customer', 'business_admin', 'business_staff', 'admin')),
...
-- Add foreign key for users.region_id
ALTER TABLE users 
ADD CONSTRAINT fk_users_region 
FOREIGN KEY (region_id) REFERENCES regions(id);

-- Add foreign key for categories.parent_id (self-referential)
ALTER TABLE categories 
ADD CONSTRAINT fk_categories_parent 
FOREIGN KEY (parent_id) REFERENCES categories(id) ON DELETE SET NULL;

-- Add foreign key for businesses.user_id
...
-- Note: This assumes there's already a users table with email/password authentication
-- You may need to adjust this based on your actual user creation logic
INSERT INTO users (email, name, role, email_verified, is_banned) 
VALUES ('admin@businessdirectory.com', 'Administrator', 'admin', true, false)
...
DO $$
BEGIN
  RAISE NOTICE 'Business directory schema migration completed successfully';
  RAISE NOTICE 'Created: 8 new tables, 22 indexes, 3 triggers';
  RAISE NOTICE 'Inserted: 5 regions, 25 categories, 1 admin user';
END $$;