...
object AuthServiceSpec extends ZIOSpecDefault:
  ...
  val testLayer =
    JwtSettings.live >>> JwtService.layer ++ mockUserRepository() ++ mockSessionRepository() >>> AuthService.layer
  ...
  override def spec: Spec[Any, Throwable] = suite("AuthService Spec")(
    ...
    test("register should return Left for existing email") {
      for
        authService <- ZIO.service[AuthService]
        result      <-
          authService.register(RegisterRequest("test@example.com", "password123", "Test User", None, None, None))
      yield result match
