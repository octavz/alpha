...
trait UserRepository:
  ...
  def findByRole(role: String): Task[List[User]]
...
class UserRepositoryImpl(ctx: PostgresCtx) extends UserRepository:
  ...
  override def findByRole(role: String): Task[List[User]] = 
    ZIO.attempt:
      run(query[User].filter(_.role == lift(role))).toList
  ...
  override def searchByEmailOrName(searchQuery: String): Task[List[User]] = 
    ZIO.attempt:
      ...
      run(query[User]
        .filter(u => u.email.like(lift(pattern)) || u.name.exists(_.like(lift(pattern))))
      ).toList
