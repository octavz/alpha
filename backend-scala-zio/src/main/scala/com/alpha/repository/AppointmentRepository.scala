...
trait AppointmentRepository:
  ...
  def findByStatus(status: AppointmentStatus): Task[List[Appointment]]
  def findByBusinessIdAndDate(businessId: UUID, date: LocalDate): Task[List[Appointment]]
  def findByBusinessIdAndDateAndStatus(
    businessId: UUID,
    date: LocalDate,
    status: AppointmentStatus): Task[List[Appointment]]
...
class AppointmentRepositoryImpl(ctx: PostgresCtx) extends AppointmentRepository:
  ...
  override def findByStatus(status: AppointmentStatus): Task[List[Appointment]] =
    ZIO.attempt:
      run(query[Appointment].filter(_.status == lift(status.value))).toList

  override def findByBusinessIdAndDate(businessId: UUID, date: LocalDate): Task[List[Appointment]] =
    ZIO.attempt:
      run(query[Appointment]
        .filter(a => a.businessId == lift(businessId) && a.appointmentDate == lift(date))).toList

  override def findByBusinessIdAndDateAndStatus(
    businessId: UUID,
    date: LocalDate,
    status: AppointmentStatus): Task[List[Appointment]] =
    ZIO.attempt:
      run(query[Appointment]
        .filter(a =>
          a.businessId == lift(businessId) && a.appointmentDate == lift(date) && a.status == lift(status.value))).toList

  override def findByBusinessIdAndDateRange(
    businessId: UUID,
    start: LocalDate,
    ...
    ZIO.attempt:
      run(query[Appointment]
        ...
        .filter(a => a.appointmentDate <= lift(end))).toList
