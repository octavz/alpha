...
object AuthEndpoints:
  ...
  val publicEndpoints: List[ZServerEndpoint[AuthService & AppConfig & TimeProvider, Any]] = List(
    endpoint.post.tag("Auth").summary("Register").in(base / "auth" / "register")
      ...
      .zServerLogic { req =>
        (for
          ...
          session <-
            ZIO.serviceWithZIO[AuthService](_.login(LoginUserRequest(req.email, req.password), None, None)).map(_._2)
    ...
    endpoint.post.tag("Auth").summary("Verify email").in(base / "auth" / "verify-email")
      ...
      .zServerLogic { req =>
        ZIO.serviceWithZIO[AuthService](_.verifyEmail(req)).as("Email verified").mapError(_.getMessage)
      },
    endpoint.post.tag("Auth").summary("Forgot password").in(base / "auth" / "forgot-password")
      ...
      .zServerLogic { req =>
        ZIO.serviceWithZIO[AuthService](_.forgotPassword(req)).as("Reset email sent").mapError(_.getMessage)
      },
    endpoint.post.tag("Auth").summary("Reset password").in(base / "auth" / "reset-password")
      ...
      .zServerLogic { req =>
        ZIO.serviceWithZIO[AuthService](_.resetPassword(req)).as("Password reset").mapError(_.getMessage)
      }
  )
  ...
  val secureEndpoints: List[ZServerEndpoint[AuthService & AppConfig & TimeProvider, Any]] = List(
    ...
    BaseEndpoints.secureEndpoint.get.tag("Auth").summary("Get me").in(base / "auth" / "me")
      .out(jsonBody[User])
      .serverLogic { ctx => _ =>
        ZIO.serviceWithZIO[AuthService](_.getUser(ctx.userId)).flatMap(
          ZIO.fromOption(_).orElseFail(new Exception("Not found"))).mapError(e => AuthError(e.getMessage, 404))
      },
