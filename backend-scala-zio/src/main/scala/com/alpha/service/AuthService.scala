...
object AuthService:
  val layer: ZLayer[UserRepository & SessionRepository & EmailVerificationRepository & PasswordResetRepository & TimeProvider & UUIDProvider, Nothing, AuthService] = 
...
class AuthServiceImpl(
  ...
  override def register(req: RegisterUserRequest): Task[User] =
    for
      ...
      user = User(
        ...
        name = req.name,
        phone = req.phone,
        role = req.role.getOrElse("CUSTOMER"),
  ...
  override def login(req: LoginUserRequest, userAgent: Option[String], ipAddress: Option[String]): Task[(User, UserSession)] =
    for
  ...
  override def updateProfile(userId: UUID, req: UpdateProfileRequest): Task[User] =
    for
      ...
      user <- ZIO.fromOption(userOpt).orElseFail(new Exception("User not found"))
      updated = user.copy(name = req.name.orElse(user.name), phone = req.phone.orElse(user.phone), avatarUrl = req.avatarUrl.orElse(user.avatarUrl), updatedAt = Some(timeProvider.now()))
  ...
  override def changePassword(userId: UUID, req: ChangePasswordRequest): Task[Unit] =
    for
      userOpt <- userRepo.findById(userId)
      user <- ZIO.fromOption(userOpt).orElseFail(new Exception("User not found"))
      _ <- ZIO.fail(new Exception("Current password is incorrect")).when(!checkPassword(req.currentPassword, user.passwordHash))
      hashedPassword = hashPassword(req.newPassword)
      _ <- userRepo.update(user.copy(passwordHash = hashedPassword, updatedAt = Some(timeProvider.now())))
    yield ()
