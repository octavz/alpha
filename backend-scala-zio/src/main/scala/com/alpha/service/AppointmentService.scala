...
trait AppointmentService:
  ...
  def getAppointmentsByDateRange(businessId: UUID, start: java.time.LocalDate, end: java.time.LocalDate): Task[List[Appointment]]
  def searchAppointments(businessId: Option[UUID], userId: Option[UUID], status: Option[String], date: Option[java.time.LocalDate]): Task[List[Appointment]]
  def getAvailability(businessId: UUID, date: java.time.LocalDate, serviceId: Option[UUID]): Task[List[AvailabilitySlot]]
...
class AppointmentServiceImpl(appointmentRepo: AppointmentRepository, timeProvider: TimeProvider, uuidProvider: UUIDProvider) extends AppointmentService:
  ...
  override def getAppointmentsByDateRange(businessId: UUID, start: java.time.LocalDate, end: java.time.LocalDate): Task[List[Appointment]] =
    appointmentRepo.findByBusinessIdAndDateRange(businessId, start, end)

  override def searchAppointments(businessId: Option[UUID], userId: Option[UUID], status: Option[String], date: Option[java.time.LocalDate]): Task[List[Appointment]] =
    (businessId, userId, status, date) match
      case (Some(bId), _, _, _) =>
        (status, date) match
          case (Some(s), Some(d)) => appointmentRepo.findByBusinessIdAndDateAndStatus(bId, d, s)
          case (_, Some(d)) => appointmentRepo.findByBusinessIdAndDate(bId, d)
          case (Some(s), _) => appointmentRepo.findByStatus(s)
          case _ => appointmentRepo.findByBusinessId(bId)
      case (_, Some(uId), _, _) =>
        appointmentRepo.findByCustomerId(uId)
      case (_, _, Some(s), _) =>
        appointmentRepo.findByStatus(s)
      case _ =>
        ZIO.succeed(List.empty[Appointment])

  override def getAvailability(businessId: UUID, date: java.time.LocalDate, serviceId: Option[UUID]): Task[List[AvailabilitySlot]] =
    for
  ...
  override def updateAppointment(id: UUID, req: UpdateAppointmentRequest): Task[Appointment] =
    for
      ...
      updated = appointment.copy(
        status = req.status.getOrElse(appointment.status),
