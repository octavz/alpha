...
export class HealthApiResponseProcessor {
     ...
     */
     public async healthCheckWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ApiResponseHealthResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ApiResponseHealthResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ApiResponseHealthResponse", ""
            ) as ApiResponseHealthResponse;
